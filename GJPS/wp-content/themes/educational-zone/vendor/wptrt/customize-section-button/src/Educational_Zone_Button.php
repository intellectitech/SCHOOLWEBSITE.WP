...
class Educational_Zone_Button extends WP_Customize_Section {
	...
	public function json() {
		...
		$json       = parent::json();
		$educational_zone_theme      = wp_get_theme();
		$button_url = $this->button_url;

		// Fall back to the `Theme URI` defined in `style.css`.
		if ( ! $this->button_url && $educational_zone_theme->get( 'ThemeURI' ) ) {

			$button_url = $educational_zone_theme->get( 'ThemeURI' );

		// Fall back to the `Author URI` defined in `style.css`.
		} elseif ( ! $this->button_url && $educational_zone_theme->get( 'AuthorURI' ) ) {

			$button_url = $educational_zone_theme->get( 'AuthorURI' );
		}

		$json['button_text'] = $this->button_text ? $this->button_text : $educational_zone_theme->get( 'Name' );
